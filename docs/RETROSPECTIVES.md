...
**State at handoff**: branch `refactor/test-optimization` (from `feat/security-csp-cookie-hardening` `93cdb8b`). Two commits: `20955ef` (13 files) + `deab6d7` (30 files). Full suite green: 1332 passed / 4 skipped / 3 xfailed / 1 xpassed. Pre-push gate green. Next: push → create stacked PR `refactor/test-optimization` → upstream `current` → CI green → merge parent PR #240 first, then rebase this PR, then squash-merge.
