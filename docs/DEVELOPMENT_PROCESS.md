...
- **CI fails on lockfile drift** → check whether the tool itself validates (`uv lock --check`, `pip install --dry-run`) before proposing a new script or workflow
...
1. **Commit `uv.lock`** if dependencies changed (`uv lock`; `uv lock --check` must pass).
...
| 2 — CI check | Fails in CI pipeline | CI runs `uv lock --check` (lock ↔ pyproject parity) |
...
[2026-09-10], which supersedes the old uv+pip split):

```bash
uv add <package>            # add a dependency (updates pyproject.toml + uv.lock)
uv sync                     # install all deps (main + dev)
uv sync --frozen --no-dev   # exactly what prod runs (see docs/RELEASE_CHECKLIST.md B1)
uv lock --check             # lock ↔ pyproject parity (pre-push + CI enforce this)
```
