...
Pushing the `v*.*.*` tag to `upstream` triggers `deploy_to_production.yml`:
the `deploy` job POSTs the production webhook, and — when the
`OPENCODE_ZEN_API_KEY` secret is configured — the `release` job creates a
**DRAFT** GitHub release with notes generated by the `release-notes` skill
(`.skills/release-notes/`). The draft is never auto-published: the maintainer
reviews the notes and publishes manually. Until the secret is set, only the
deploy runs; draft notes are then created manually following the skill.
