...
| B17 | **DB auto-migration (self-healing)** | `docker/entrypoint.sh` runs `python flask_se.py migrate` on boot (idempotent: fresh DB → `init_db()`/`create_all`; existing DB → backup + `ensure_schema()` PRAGMA-driven `ADD COLUMN`). No ops pre-flight needed. Disable per-deploy with `SE_AUTO_MIGRATE=0` in compose to run it manually. Verify after deploy: the app boots and `users.deleted` (or any recent column) exists — see `docs/TOOLING.md` §Auto-migrate |
