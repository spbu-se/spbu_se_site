...
| B16 | **Maps key on prod** (until moved to Yandex) | The 3 map pages (index, contacts, bachelor_admission) render maps only when `SE_GOOGLE_MAPS_KEY`/`configs/flask_se_maps.conf` is present on the deploy host. Post-deploy verify `SE_GMAPS_KEY` is non-empty on one of those pages (`curl -s .../contacts.html | grep -o 'SE_GMAPS_KEY = "[^"]*"'`); empty → maps render as blank 500px boxes |
