...
Shared role/entity fixtures (`make_theme`, `admin_client`, `reviewer_client`, …) are defined **once** in `tests/conftest.py` — never copy a fixture into a new test module (duplicates trip the CI pylint-similarities check R0801). Canonical policy: `[[TESTING.md#Testing-Discipline]]`.
