...
class AddThesisOnReview(FlaskForm):
    ...
    author = StringField(
        "author",
        description="Ваше полное ФИО. Например, Иванов Иван Иванович",
    )
    thesis = FileField()
    author = StringField(
        "author",
...
class ChooseTopic(FlaskForm):
    ...
    consultant = StringField(
        "consultant",
        description="ФИО консультанта, должность и компания",
    )
...
class AddTask(FlaskForm):
    task_text = StringField(
        "task_text",
        description="Например, научиться работать с ajax.",
    )
