...
@login_required
def practice_guide():
    return render_template(PracticeStudentTemplates.GUIDE.value, thesises=get_list_of_theses())
...
def practice_add_new_report(current_thesis):
    if request.method == "POST":
        ...
        if not was_done:
            flash(
                'Поле "Что было сделано?" является обязательным!',
                category="error",
            )
    ...
    if request.method == "POST":
        if "submit_text_button" in request.form:
            ...
            if "text_link" in request.form and request.form["text_link"] != "":
                ...
                db.session.commit()
                flash(
                    "Ссылка на текст работы сохранена!",
                    category="success",
                )
            ...
            if (
                ...
                flash(
                    "Текст отзывов должен быть в формате .PDF",
                    category="error",
                )
                return redirect(url_for("practice_preparation", id=current_thesis.id))
            ...
            if supervisor_review:
                ...
                db.session.commit()
                flash(
                    "Отзыв научного руководителя успешно загружен!",
                    category="success",
                )
            ...
            if reviewer_review:
                ...
                db.session.commit()
                flash(
                    "Отзыв консультанта успешно загружен!",
                    category="success",
                )
            ...
            if presentation_file is None and (
                ...
                flash(
                    "Вы не указали ссылку на презентацию.",
                    category="error",
                )
                return redirect(url_for("practice_preparation", id=current_thesis.id))
            ...
            if "presentation_link" in request.form and request.form["presentation_link"] != "":
                ...
                db.session.commit()
                flash(
                    "Ссылка на презентацию сохранена!",
                    category="success",
                )
            ...
            if code_link not in {None, ""}:
                ...
                db.session.commit()
                flash(
                    "Ссылка на репозиторий сохранена!",
                    category="success",
                )
