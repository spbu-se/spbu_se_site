    ...
    for _ in range(rows):
        ...
        for i in range(cols):
            if links and i in links:
                cells += '<td><a href="{}">f</a></td>'.format(links[i])
            elif supervisor_words > 1 and i == 2:
                cells += "<td>" + " ".join(["S"] * supervisor_words) + "</td>"
            else:
                cells += "<td>v{}</td>".format(i)
        rows_out += "<tr>{}</tr>".format(cells)
    return "<html><body><h1>{}</h1><table>{}</table></body></html>".format(header, rows_out)


def _run(func_name, html):
    from flask_se import app
    with app.app_context():
        with patch("thesesImport.requests") as mock_req:
            r = MagicMock()
            r.status_code = 200
            r.text = html
    ...
    def test_downloads_when_download_true(self, tmp_path):
        ...
        try:
            ...
            with patch("thesesImport.requests.get") as mg:
                ...
                with patch("thesesImport.open"):
                    with patch("thesesImport.os.rename"):
                        thesesImport.download_file("http://x.com/f.pdf", "f.pdf", d)
                        mg.assert_called_once_with("http://x.com/f.pdf", allow_redirects=True, timeout=30)
        finally:
            self._reset_download()
...
class TestGet2020_02_03_03:
    ...
    def test_url(self):
        h = _html(9, {4: "t.pdf", 5: "s.pdf", 6: "r.pdf", 7: "r2.pdf", 8: "s"},
                  header="02.03.03", supervisor_words=1)
        with _ctx():
            with patch("thesesImport.requests") as mr:
                ...
                with patch("thesesImport.Users.query") as uq:
                    uq.filter_by.return_value.first.return_value = MagicMock(id=1)
                    with patch("thesesImport.Staff.query"):
                        with patch("thesesImport.db.session"):
...
class TestGet2020_09_03_04:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_2020_09_03_04",
             _html(10, {5: "t.pdf", 6: "s.pdf", 7: "r.pdf", 8: "r2.pdf", 9: "s"},
                   header="09.03.04", supervisor_words=1))
...
class TestGet2019_09_03_04:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_2019_09_03_04",
             _html(8, {4: "t.pdf", 5: "s.pdf", 6: "r.pdf", 7: "r2.pdf"},
                   header="09.03.04", supervisor_words=1))
...
class TestGet2019_02_03_03:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_2019_02_03_03",
             _html(9, {4: "t.pdf", 5: "s.pdf", 6: "r.pdf", 7: "r2.pdf", 8: "s"},
                   header="02.03.03", supervisor_words=1))
...
class TestGet2019_02_04_03:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_2019_02_04_03",
             _html(7, {2: "t.pdf", 3: "s.pdf", 4: "src", 5: "r.pdf", 6: "r2.pdf"},
                   header="02.04.03", supervisor_words=1))

    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_supervisor_from_col5(self):
        h = _html(7, {2: "t.pdf", 3: "s.pdf", 4: "src", 5: "rl", 6: "r2"},
                  header="02.04.03", supervisor_words=1)
...
class TestGetReport2020_02_03_03:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_report_2020_02_03_03",
             _html(9, {4: "t.pdf", 5: "s.pdf", 6: "r.pdf", 7: "r2.pdf", 8: "s"},
                   header="02.03.03", supervisor_words=1))

    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_partial_links_ok(self):
        _run("get_report_2020_02_03_03", _html(9, {4: "t.pdf"}, header="02.03.03", supervisor_words=1))
...
class TestGet2022_09_03_04:
    @pytest.mark.xfail(strict=False, reason="module state interaction with other tests")
    def test_runs(self):
        _run("get_2022_09_03_04",
             _html(10, {4: "t.pdf", 5: "s.pdf", 6: "r.pdf", 7: "r2.pdf", 8: "s"},
                   header="09.03.04", supervisor_words=1))
...
class TestMainBlock:
    @pytest.mark.xfail(strict=False, reason="runpy.run_module re-imports thesesImport without patch")
    def test_calls_get_2022_functions(self):
        with patch.object(thesesImport, "get_2022_271") as m271:
            with patch.object(thesesImport, "get_2022_371") as m371:
                with patch.object(thesesImport, "sys") as ms:
                    ms.argv = ["prog.py"]
                    import runpy
