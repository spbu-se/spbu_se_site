...
class TestDiplomasDeep:
    ...
    def test_delete_theme_no_id(self, logged_client):
        assert_ok(logged_client, "/diplomas/delete_theme.html", code={302})

    def test_delete_theme_success(self, diploma_theme):
        resp = diploma_theme.get("/diplomas/delete_theme.html?theme_id=1")
        assert resp.status_code in (200, 302)

    def test_archive_theme(self, diploma_theme):
        resp = diploma_theme.get("/diplomas/archive_theme?theme_id=1")
        assert resp.status_code in (200, 302)

    def test_unarchive_theme(self, diploma_theme):
        resp = diploma_theme.get("/diplomas/unarchive_theme?theme_id=1")
        assert resp.status_code in (200, 302)
    ...
    def test_delete_theme_created_by_user(self, logged_client):
        ...
        db.session.commit()
        theme_id = theme.id
        resp = logged_client.get(f"/diplomas/delete_theme.html?theme_id={theme_id}")
        assert resp.status_code in (200, 302)
    ...
    def test_archive_theme_owned_by_user(self, logged_client):
        ...
        db.session.commit()
        theme_id = theme.id
        resp = logged_client.get(f"/diplomas/archive_theme?theme_id={theme_id}")
        assert resp.status_code in (200, 302)
    ...
    def test_unarchive_theme_owned_by_user(self, logged_client):
        ...
        db.session.commit()
        theme_id = theme.id
        resp = logged_client.get(f"/diplomas/unarchive_theme?theme_id={theme_id}")
        assert resp.status_code in (200, 302)
